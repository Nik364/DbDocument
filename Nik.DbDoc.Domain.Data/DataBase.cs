        ...
        public IList<Model.DataBase> GetList()
        {
            string sql = @"
                ...
                SELECT @sql = LEFT(Tmp, LEN(Tmp) - 10)  FROM (
                    SELECT (
                        SELECT 'SELECT CAST(p.Value AS NVARCHAR(128)), ''' + name + ''' Name FROM ' + name + '.sys.extended_properties p WHERE p.class = 0 AND p.name = ''MS_Description'' UNION ALL ' 
                        FROM master..sysdatabases FOR XML PATH('')
                    ) Tmp
                ) DbCaption;
